...
for i=1:offnr
    ...
    cellmat((i*offsize-offsize+1):(i*offsize),3)=1/(1+sc); % mutation, sc
    cellmat((i*offsize-offsize+1):(i*offsize),4)=sg; % mutation, sg
...
if mod(initpop,offsize)>0
    ...
    cellmat((i*offsize+1):(i*offsize+lastsize),3)=1/(1+sc); % mutation, sc
    cellmat((i*offsize+1):(i*offsize+lastsize),4)=sg; % mutation, sg
...
% reproduction times
for j=1:initpop
    rn=normrnd(0,.1,1); % distribute time
    tnext=ceil((rn+2+cellmat(j,3))/dt);
    %tnext=ceil(abs(30*randn)+(cellmat(j,3))/dt); 
